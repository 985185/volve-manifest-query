...
def _split_tags(tags: Any) -> List[str]:
    ...
    if isinstance(tags, str):
        # manifests often store "DOCS|WELL_TECH"
...
def _filename_from_path(path: str) -> str:
    # manifest paths are dbfs:/... or similar; treat as posix
    try:
...
class ManifestIndex:
    ...
    def _build(self) -> None:
        ...
        for well_key, manifest in self.manifests.items():
            well_id = manifest.resolved_well_id() or well_key.replace("_", "/")
            for bucket_name, entries in (manifest.buckets or {}).items():
                for e in entries or []:
                    path = str(e.get("path", ""))
                    if not path:
                        continue
                    etype = str(e.get("type", "file"))
                    filename = str(e.get("filename") or e.get("name") or _filename_from_path(path))
                    tags = _split_tags(e.get("tags"))
                    flat.append(
                        SearchHit(
                            type=etype,
                            well_key=well_key,
    ...
    def summary(self, well_key: str) -> Dict[str, Any]:
        ...
        well_id = m.resolved_well_id() or well_key.replace("_", "/")
    ...
    def bucket_files(self, well_key: str, bucket: str) -> List[SearchHit]:
        # Use flat index so output schema is consistent
    ...
    def foreign_refs(self, well_key: str) -> List[Dict[str, Any]]:
    ...
    def search(
        ...
        def hit(h: SearchHit) -> bool:
            ...
            # Search in: filename, path, bucket, well_key, well_id, tags
            ...
            # Also: normalize separators so 15_9-F-9A can match 15/9-F-9A
